 ...
 */
public class CarGoOrderActivity extends BaseActivity {
    private static final String TAG="CarGoOrderActivity";
    ...
    private MyHandler handler =new MyHandler(this) {
        ...
        public void handleMessage(Message msg) {
            ...
            if(msg.what ==1) {
                ...
                try {
                    ...
                    if (status == 0) {
                        ...
                        if (json.getString("Cargos").equals("[]")){
                            //MyToast.makeText(CarGoOrderActivity.this, "暂时没有抢单记录");
                            switch (requestType){
                                case 1:
                        ...
                        isCompleated = true;
                        MyToast.makeText(CarGoOrderActivity.this, "亲、目前没有订单");
                        switch (requestType){
                            case 1:
                ...
                isCompleated = true;
                MyToast.makeText(CarGoOrderActivity.this, msg.what+"服务器异常");
                switch (requestType){
                    case 1:
                        recordPullToRefreshLayout.refreshFinish(PullToRefreshLayout.FAIL);
                        break;
    ...
    private void init() {
        ...
        xiala.setOnRefreshListener(new PullToRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh(PullToRefreshLayout pullToRefreshLayout) {
                ...
                if (GlobalParams.isNetworkAvailable(CarGoOrderActivity.this)) {
                    if (isCompleated) {
                        mlist.clear();
                        adapter.notifyDataSetChanged();
                        currentPage = 1;
                        ...
                        MyToast.makeText(CarGoOrderActivity.this, "正在加载中。。。");
                    }
                } else {
                    isCompleated = true;
                    pullToRefreshLayout.refreshFinish(PullToRefreshLayout.FAIL);
                    Toast.makeText(CarGoOrderActivity.this, "亲,请连接网络！！！", Toast.LENGTH_SHORT)
                            .show();
                }
        ...
        listView.setOnLoadListener(new PullableListView.OnLoadListener() {
            ...
            public void onLoad(PullableListView pullableListView) {
                ...
                if (GlobalParams.isNetworkAvailable(CarGoOrderActivity.this)) {
                    ...
                    pullableListView.finishLoading();
                    MyToast.makeText(CarGoOrderActivity.this, "亲,请连接网络！！！");
    ...
    public void NetRefresh(){
        ...
        try {
            ...
            isCompleated=false;
            new MyThread(Constant.URL_PostGetCarOrder,handler,json,CarGoOrderActivity.this).start();
